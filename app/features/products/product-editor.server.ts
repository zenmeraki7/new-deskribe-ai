...
export async function action({ request, params }: ActionFunctionArgs): Promise<Response> {
  ...
  const existing = await db.generationJob.findFirst({
    ...
    if (existing) {
      appLog.info("Product generation reused existing job", {
    ...
    if (!credit.allowed) {
      appLog.warn("Product generation blocked by credit limit", {
        operation: "product.generate",
        shop: shopDomain,
        requestId: creditRequestId,
        productId: productGid,
    ...
    appLog.info("Product generation queued", {
      operation: "product.generate",
      shop: shopDomain,
      jobId: jobIds[0],
      requestId: creditRequestId,
    ...
    appLog.error("Product generation enqueue failed", {
      operation: "product.generate",
      shop: shopDomain,
      requestId: creditRequestId,
      productId: productGid,
  ...
  if (intent === "apply") {
    ...
    if (!parsed) {
      appLog.warn("Product apply failed because draft was unavailable", {
        operation: "product.apply",
        shop: shopDomain,
        jobId,
        productId: productGid,
        durationMs: durationSince(startedAt),
    ...
    if (Array.isArray(userErrors) && userErrors.length > 0) {
      ...
      appLog.warn("Product apply rejected by Shopify", {
        operation: "product.apply",
        shop: shopDomain,
        jobId,
        productId: productGid,
    ...
    appLog.info("Product apply completed", {
      operation: "product.apply",
      shop: shopDomain,
      jobId,
      productId: productGid,
  ...
  if (intent === "suggest_keywords") {
    ...
    try {
      ...
      if (!credit.allowed) {
        appLog.warn("Keyword suggestion blocked by credit limit", {
          operation: "product.suggest_keywords",
          shop: shopDomain,
          requestId: keywordCreditRequestId,
          productId: productGid,
          durationMs: durationSince(startedAt),
      ...
      appLog.info("Keyword suggestion completed", {
        operation: "product.suggest_keywords",
        shop: shopDomain,
        requestId: keywordCreditRequestId,
        productId: productGid,
      ...
      appLog.error("Keyword suggestion failed", {
        operation: "product.suggest_keywords",
        shop: shopDomain,
        requestId: keywordCreditRequestId,
        productId: productGid,
