...
import type {
  ...
  ProductMeta,
  DraftResult,
} from "../../routes/app.products.$productId.types";
import {
  checkRateLimit,
  incrementRateLimit,
...
function normalizeProductGid(raw: string): string | null {
  ...
  if (SHOPIFY_NUMERIC_ID_RE.test(decoded))
...
function parseDraftResultOrNull(value: unknown): DraftResult | null {
  ...
  const cappedKeywords = r.keywords
    ? normalizeKeywordList(r.keywords)
    : undefined;
...
export async function loader({
  ...
  if (latestDraft?.result) {
    ...
    const pk =
      typeof parsed?.primary_keyword === "string"
  ...
  const latestOutput = latestDraft
    ? await db.generatedSeoOutput.findFirst({
        where: {
          shopDomain,
          jobId: latestDraft.id,
          productId: productGid,
          status: "READY",
        },
        select: { fields: true },
  ...
  const sanitizedLatestDraft: LoaderData["latestDraft"] = latestDraft
    ? {
        ...
        result: (() => {
          ...
          const outputFields =
            latestOutput?.fields && typeof latestOutput.fields === "object"
              ? (latestOutput.fields as Record<string, unknown>)
              : {};
          const outputBody =
          ...
          const kw = parsed.keywords
            ? normalizeKeywordList(parsed.keywords)
            : undefined;
...
export async function action({
  ...
  const { admin, billing, shopDomain } = authContext;
      ...
      return json(
        {
          ...
          kind: "error",
          error:
            "Custom writing styles require an Advanced or Pro plan. Upgrade to unlock this feature.",
  ...
  if (intent === "delete_template") {
    ...
    await db.customTemplate.deleteMany({
      where: { id: templateId, shopDomain },
    });
  ...
  if (intent === "generate") {
    ...
    try {
      const { jobIds, skipped, deduplicated } = await enqueueGenerationJobs({
        shopDomain,
      ...
      if (deduplicated && jobIds[0]) {
        await refundCredits({
          shopId: shopDomain,
          plan,
          amount: CREDIT_COSTS.standardGeneration,
          requestId: `${creditRequestId}:deduplicated`,
          metadata: { intent: "generate", productId: productGid },
        });
        return json({
          ok: true,
      ...
      await incrementRateLimit(shopDomain, plan).catch((error) => {
        console.warn("[generate] rate-limit increment skipped:", error);
      });
  ...
  if (intent === "apply") {
      ...
      return json(
        {
          ok: false,
          kind: "error",
          error: "Draft not found or invalid",
    ...
    try {
      const applyResult = await enqueueApplyJob({
        shopDomain,
        jobId,
      ...
      return json(
        {
          ok: false,
          kind: "error",
          error: "Apply could not be queued",
          code: "APPLY_NOT_QUEUED",
        },
        { status: 500 },
      );
    }
  ...
  if (intent === "suggest_keywords") {
    ...
    try {
      ...
      const limitResult = await checkAndIncrementKeywordLimit(
        shopDomain,
        keywordPlan,
      );
  ...
  if (intent === "fetch_description") {
    ...
    const descriptionHtml: string = String(
      gql.data?.product?.descriptionHtml ?? "",
    );
