...
import { buildKeywordRegex, countRegexMatches, normalizeKeywords } from "./keywords";
...
function highlightHtmlWithKeywords(html, keywords) {
  ...
  const re = buildKeywordRegex(safeKeywords);
  if (!re) {
    try {
      const doc = new DOMParser().parseFromString(safeHtml, "text/html");
      sanitizeDocAllowlist(doc);
  ...
  try {
    ...
    const approxMatches = countRegexMatches(preText, re, LIMITS.MAX_KEYWORD_MATCHES_PER_DOC);
