...
export function DiffViewer({
  ...
  const before = beforeTooLarge
    ? clampString(beforeRaw, MAX_RENDER_CHARS)
    : beforeRaw;
  const after = afterTooLarge
    ? clampString(afterRaw, MAX_RENDER_CHARS)
    : afterRaw;
    ...
    <Card padding="0">
      ...
      {(beforeTooLarge || afterTooLarge) && (
        <Box paddingInline="400" paddingBlockEnd="200">
          <Banner tone="warning">
            <p>
              Content is very large and has been truncated for safe preview. Use
              server-side versioning for full fidelity review and apply.
            </p>
          </Banner>
        </Box>
      )}
