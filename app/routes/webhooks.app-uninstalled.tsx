...
export const action = async ({ request }: ActionFunctionArgs) => {
  ...
  try {
    ...
    if (activeJobs.length > 0) {
      ...
      console.log(
        `[uninstall] Cancelled ${activeJobs.length} active jobs for ${shop}`,
      );
    }

    // Step 3: Delete all DB rows
    await db.$transaction([
      db.applyJob.deleteMany({ where: { shopDomain: shop } }),
      db.productMutationSnapshot.deleteMany({ where: { shopDomain: shop } }),
      db.productSeoSnapshot.deleteMany({ where: { shopDomain: shop } }),
      db.generatedSeoOutput.deleteMany({ where: { shopDomain: shop } }),
      db.generationJob.deleteMany({ where: { shopDomain: shop } }),
      db.bulkOperation.deleteMany({ where: { shopDomain: shop } }),
