  ...
  return (
    ...
    >
      <Modal.Section>
        <BlockStack gap="400">
          {applyError && (
            <Banner tone="critical" title="Apply failed">{applyError}</Banner>
          )}
          {applySuccess && (
...
export default function BulkReviewPage() {
  ...
  const applyFetcher = useFetcher<{ ok: boolean; error?: string; intent?: string }>();
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (stopped) return;
      if (document.hidden) { timerRef.current = window.setTimeout(tick, scheduleMs()); return; }
  ...
  }, [applyAllFetcher.data, revalidator]);
  ...
  const unappliedCompleted = jobs.filter(
    (j) => j.status === "COMPLETED" && j.appliedAt === null && !appliedJobIds.has(j.id),
  );
  ...
  const previewJobCurrent =
    previewJob ? (jobs.find((j) => j.id === previewJob.id) ?? previewJob) : null;
        ...
        <Card>
          <BlockStack gap="300">
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {[
                { label: "Total", value: totalCount, bg: "#f6f6f7", color: "#202223" },
                { label: "Completed", value: completedCount, bg: "#f0fdf4", color: "#1a7f37" },
                { label: "In Progress", value: pendingCount, bg: "#fff8e1", color: "#916a00" },
                { label: "Failed", value: failedCount, bg: "#fff5f5", color: "#c0392b" },
                { label: "Applied", value: appliedCount + appliedJobIds.size, bg: "#eff6ff", color: "#2c6ecb" },
                ...
                >
                  ...
                  <span style={{ fontSize: 11, color: "#6d7175", marginTop: 2, fontWeight: 500 }}>{label}</span>
                </div>
              ))}
            </div>
          </BlockStack>
      ...
      <PreviewModal
        ...
        applySuccess={
          ...
          (appliedJobIds.has(previewJob.id) ||
            (applyFetcher.data?.ok === true && (applyFetcher.data as any).jobId === previewJob.id))
        }
        applyError={
          previewJob !== null &&
