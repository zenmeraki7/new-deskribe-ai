...
export default function ProductEditorModalRoute() {
  ...
  const [includeSocials] = useState<boolean>(false);
  ...
  const canUseCustomTemplates = shopPlan === "standard" || shopPlan === "pro";
  ...
  }, [generateFetcher.data, generateFetcher.data?.jobId, startPolling]);
  ...
  }, [activeJob, activeJob?.id, activeJob?.status, startPolling]);
  ...
  const generateError =
    (isCreditLimitError ? creditLimitMessage : "") ||
    (generateFetcher.data?.ok === false ? String(generateFetcher.data.error ?? "") : "");
