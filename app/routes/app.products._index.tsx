...
export default function ProductsDashboard() {
  ...
  const affordableBulkCount = Math.min(selectedResources.length, credits.creditsRemaining);
  const canGenerateAnyBulk = affordableBulkCount > 0;
      ...
      : [
          {
            ...
            onAction: () => {
              if (!canGenerateAnyBulk) return;
              setBulkModalOpen(true);
            },
            disabled: !canGenerateAnyBulk,
          },
        ];
            ...
            {selectedResources.length > 0 && (
              <Box paddingInline="400" paddingBlockStart="200">
                {!canGenerateAnyBulk ? (
                  ...
                  <Banner
                    tone="warning"
                    title="Partial generation available"
                    action={{
                      content: "Upgrade plan",
                      url: "/app/billing",
                      target: "_self",
                  ...
                  >
                    {formatCredits(affordableBulkCount)} product
                    {affordableBulkCount === 1 ? "" : "s"} will be queued now.{" "}
                    {formatCredits(selectedResources.length - affordableBulkCount)} product
                    {selectedResources.length - affordableBulkCount === 1 ? "" : "s"} will not
                    be touched until you upgrade.
                  </Banner>
                ) : (
