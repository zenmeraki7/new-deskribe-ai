...
export function shouldRevalidate({
  ...
  if (
    ...
    "ok" in actionResult &&
    typeof actionResult.ok === "boolean"
  ) {
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const rawCredits = await getCreditBalance(shopDomain, shopPlan);
  const credits = {
    ...rawCredits,
    creditsUsed: Math.max(0, safeNumber(rawCredits.creditsUsed)),
    creditsLimit: Math.max(0, safeNumber(rawCredits.creditsLimit)),
    creditsRemaining: Math.max(0, safeNumber(rawCredits.creditsRemaining)),
  };
...
export default function ProductsDashboard() {
  ...
  const creditsUsed = Math.max(0, credits.creditsLimit - creditsRemaining);
  ...
  const handleBulkError = useCallback(
    (error: { code: string; message: string; productCount: number }) => {
      console.error("[bulk-generate] client failure", error);
    },
    [],
  );
  ...
  const hasEnoughBulkCredits = hasCredits(creditsRemaining, bulkCreditCost);
  const isWithinBulkLimit = selectedResources.length <= MAX_BULK_PRODUCT_COUNT;
    ...
    : [
        {
          ...
          onAction: () => {
            if (!hasEnoughBulkCredits || !isWithinBulkLimit) return;
            setBulkModalOpen(true);
          },
          disabled: !hasEnoughBulkCredits || !isWithinBulkLimit,
        },
      ];
  ...
  return (
    ...
    >
              ...
              <Box paddingInline="400" paddingBlockStart="200">
                {!isWithinBulkLimit ? (
                  <Banner tone="critical" title="Too many products selected">
                    Select no more than {MAX_BULK_PRODUCT_COUNT} products per
      ...
      <BulkGenerateModal
        ...
        onCreditBalanceChange={setCreditsRemaining}
        creditsRemaining={creditsRemaining}
        monthlyCreditsUsed={creditsUsed}
        monthlyCreditLimit={credits.creditsLimit}
