...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    if (data?.errors?.length) {
      loadError = data.errors
        .map((error: { message?: string }) => error.message)
        .filter(Boolean)
        .join("; ");
    }

    productData = data?.data?.products ?? productData;
    collectionEdges = data?.data?.collections?.edges ?? [];
  } catch (error) {
    console.error("[products] Failed to load Shopify products:", error);
    loadError =
  ...
  const collections = collectionEdges.map((edge: any) => edge.node.title);
